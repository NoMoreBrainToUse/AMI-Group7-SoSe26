...
def build_rgb_timeline(
    ...
    # FRED annotations normally index the usable RGB/PADDED_RGB sequence, not
    # the earlier Removed_frames folder. Keep the removed-frame path available
    # as an opt-in diagnostic for unusual exports, but do not use it by default.
    if include_removed:
        removed_paths = list_images(seq_dir / "Removed_frames")
        combined_names = sorted({p.name for p in rgb_paths} | {p.name for p in removed_paths}, key=lambda n: natural_key(Path(n)))
    else:
        combined_names = [p.name for p in rgb_paths]
    index_by_name = {name: index for index, name in enumerate(combined_names)}

    timeline: list[TimedPath] = []
    for path in rgb_paths:
        index = index_by_name.get(path.name)
        if index is None:
            continue
        timeline.append(TimedPath((index + 1) * frame_period, path))
...
def process_sequence(
    ...
    rgb_timeline = build_rgb_timeline(seq_dir, rgb_dir_name, frame_period, include_removed_in_rgb_timeline)
...
def write_yolo_yaml(output_root: Path, dataset_name: str) -> None:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--train-seqs", default="0,1,11,12,13,101,102,103")
    parser.add_argument("--val-seqs", default="10,100")
    parser.add_argument("--test-seqs", default="110")
...
def main() -> int:
    ...
    all_manifests: dict[str, list[dict[str, str]]] = {"train": [], "val": [], "test": []}
    ...
    for split, rows in all_manifests.items():
        write_manifest(output_root, split, rows)

    write_yolo_yaml(output_root, "rgb_yolo")
    write_yolo_yaml(output_root, "event_yolo")
    ...
    report = {
        ...
        "totals": {
            split: len(rows) for split, rows in all_manifests.items()
        },
    }
