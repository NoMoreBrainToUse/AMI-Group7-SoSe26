...
Default input:
  processed/fred_subset/event_yolo/data.yaml

Example:
  python src/training/train_event_yolo.py --epochs 100 --imgsz 640 --batch 16

Use a larger YOLO11 checkpoint if the GPU has enough memory:
  python src/training/train_event_yolo.py --model yolo11s.pt --epochs 150
"""
...
def build_parser() -> argparse.ArgumentParser:
    root = repo_root()
    default_data = root / "processed" / "fred_subset" / "event_yolo" / "data.yaml"
    ...
    parser.add_argument("--name", default="fred_subset_event_yolo11m", help="Run name under --project.")
