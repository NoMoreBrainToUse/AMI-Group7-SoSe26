...
Example:
  python src/training/predict_event_yolo_val.py \
    --model runs/event_yolo/fred_subset_event_yolo11n/weights/best.pt \
    --source processed/fred_subset/event_yolo/images/val \
    --output outputs/fred_subset_event_yolo_val_pred \
    --device 0
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--model",
        default=str(root / "runs" / "event_yolo" / "fred_subset_event_yolo11n" / "weights" / "best.pt"),
        help="Path to YOLO weights.",
    )
    parser.add_argument(
        "--source",
        default=str(root / "processed" / "fred_subset" / "event_yolo" / "images" / "val"),
