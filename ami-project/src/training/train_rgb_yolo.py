#!/usr/bin/env python3
"""
Train a YOLO11 detector on the preprocessed FRED RGB dataset.

Default input:
  processed/fred_subset/rgb_yolo/data.yaml

Example:
  python src/training/train_rgb_yolo.py --epochs 100 --imgsz 640 --batch 16

Use a larger YOLO11 checkpoint if the GPU has enough memory:
  python src/training/train_rgb_yolo.py --model yolo11l.pt --epochs 150
"""
...
def validate_rgb_dataset(data_yaml: Path) -> None:
    ...
    if missing:
        raise FileNotFoundError("Missing RGB YOLO directories:\n  " + "\n  ".join(missing))

    print(f"RGB dataset: {dataset_dir}")
...
def build_parser() -> argparse.ArgumentParser:
    root = repo_root()
    default_data = root / "processed" / "fred_subset" / "rgb_yolo" / "data.yaml"
    default_project = root / "runs" / "rgb_yolo"
    default_workers = 0 if os.name == "nt" else 8

    parser = argparse.ArgumentParser(
        description="Train Ultralytics YOLO11 on processed FRED RGB frames.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--data", default=str(default_data), help="Path to rgb_yolo/data.yaml.")
    parser.add_argument(
        "--model",
        default="yolo11m.pt",
    ...
    parser.add_argument("--name", default="fred_subset_rgb_yolo11m", help="Run name under --project.")
...
def train_or_validate(args: argparse.Namespace) -> None:
    ...
    validate_rgb_dataset(data_yaml)
    ...
    try:
        ...
        raise SystemExit(
            "Could not import ultralytics. Activate the virtual environment where YOLO11 is installed, "
